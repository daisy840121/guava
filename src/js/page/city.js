...
$(function () {
  ...
  const edd_county = easydropdown('#county', {
    behavior: {
      liveUpdates: true
    }
  });
  const edd_district = easydropdown('#district', {
    behavior: {
      liveUpdates: true
    }
  });

  // 台灣區域 方法2 (不使用twCitySelector.js)
  // let tw = {
  ...
  // $('#county').append(`<option disabled selected>選擇縣市</option>`)
  // let cityLength = tw.counties.length;
  // let array = tw.counties
  // for (let i = 0; i < cityLength; i++) {
  //   $('#county').append(`<option value="${array[i]}">${array[i]}</option>`)
  // };
  ...
  // $('.counties .edd-option').on('click', function (event) {
  //   let index = parseInt($(this).index()) - 1;
  //   let districtLength = tw.districts[index][0].length;
  //   let array = tw.districts[index][0];
  //   $('#district').html('')
  //   $('#district').append(`<option disabled selected>選擇區域</option>`)
  //   for (let i = 0; i < districtLength; i++) {
  //     $('#district').append(`<option value="${array[i]}">${array[i]}</option>`)
  //   }
  // });
});